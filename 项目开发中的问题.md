...
# 4. 初始显示抛异常
    1). 问题: Cannot read property 'xxx' of undefined"
    2). 原因: 初始状态值为一个空对象: obj,  obj.xxx.yyy (三层表达式)
    3). 解决: 使用v-if判断(不能用v-show) , 如果obj中没有数据就直接不解析/不显示

    1). 问题2: Cannot read property 'bgImg' of null"
    2). 原因: 初始状态值为一个null:  obj.xxx (二层表达式)
    3). 解决: 初始状态值为一个空对象/数组

# 5. 使用better-scroll库的问题
    1). 问题: 创建BScroll对象后不能滑动
    2). 原因: 创建BScroll对象的时机太早了, 必须要在列表数据更新显示之后创建
    3). 解决: 可以有3种解决办法---想办法在列表数据更新显示之后才去创建BScroll对象
        a. 使用watch+$nextTick( () =>{界面更新之后立即执行})
        b. 使用回调+$nextTick()
        c. 利用dispatch()返回的promise
